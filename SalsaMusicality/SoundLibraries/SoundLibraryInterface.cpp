...
int InitializeSoundLibrary(char *ipPathToSoundFonts)
{
    ...
    g_pInstruments[INSTRUMENT_CONGAS] =
        new CInstrumentCongas(pathsToSoundFontFiles[INSTRUMENT_CONGAS], true, CON_BASIC_FULL);

    g_pCollection->AddInstrumentToCollection(g_pInstruments[INSTRUMENT_CONGAS]);

    /* Initialize clave */
    g_pInstruments[INSTRUMENT_CLAVE] =
        new CInstrumentClave(pathsToSoundFontFiles[INSTRUMENT_CLAVE], false, CLA_SON_2_3);

    g_pCollection->AddInstrumentToCollection(g_pInstruments[INSTRUMENT_CLAVE]);

    /* Initialize cowbell */
    g_pInstruments[INSTRUMENT_COWBELL] =
        new CInstrumentCowbell(pathsToSoundFontFiles[INSTRUMENT_COWBELL], false, COW_DOWN_BEATS);

    g_pCollection->AddInstrumentToCollection(g_pInstruments[INSTRUMENT_COWBELL]);

    /* Initialize bongos */
    g_pInstruments[INSTRUMENT_BONGOS] =
        new CInstrumentBongos(pathsToSoundFontFiles[INSTRUMENT_BONGOS], false, BON_MARTILLO);
